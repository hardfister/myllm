...
const loadMeta = (): Record<number, { title: string; color: string }> => { try { const r = localStorage.getItem(META_KEY); return r ? JSON.parse(r) : {} } catch { return {} } }
const saveMeta = (m: Record<number, { title: string; color: string }>) => { localStorage.setItem(META_KEY, JSON.stringify(m)) }
...
const loadModelsData = async () => {
  ...
  let mc = false
  for (const m of models.value) { if (m.id != null && !frontendMeta.value[m.id]) { frontendMeta.value[m.id] = { title: m.displayName || `${m.provider} - ${m.modelName}`, color: '#1e3a8a' }; mc = true } }
...
const saveFinalConfig = async (fd: { title: string; color: string; maxTokens: number; prompt: string }) => {
  ...
  const p: ModelConfig = { modelName: currentConfig.value.modelName, provider: currentConfig.value.provider, apiKeyEncrypted: currentConfig.value.apiKeyEncrypted, baseUrl: currentConfig.value.baseUrl, maxTokens: currentConfig.value.maxTokens, prompt: currentConfig.value.prompt, displayName: currentConfig.value.displayName || currentConfig.value.title, sortOrder: isEditing.value ? (currentConfig.value.id != null ? models.value.find(m => m.id === currentConfig.value.id)?.sortOrder ?? 0 : models.value.length) : models.value.length, isEnabled: 0 }
  try {
    if (useServer()) { if (isEditing.value && currentConfig.value.id != null) { await updateModel(currentConfig.value.id, p) } else { const r = await createModel(p); if (r.data.id != null) currentConfig.value.id = r.data.id }; await loadModelsData() }
    else { if (isEditing.value && currentConfig.value.id != null) { const i = models.value.findIndex(m => m.id === currentConfig.value.id); if (i !== -1) models.value[i] = { ...models.value[i], ...p } } else { models.value.push({ ...p, id: ++localIdCounter, sortOrder: models.value.length }) }; persistModels() }
    if (currentConfig.value.id != null) { frontendMeta.value[currentConfig.value.id] = { title: currentConfig.value.title || p.modelName, color: currentConfig.value.color }; saveMeta(frontendMeta.value) }
    emit('updateColor', currentConfig.value.color); step.value = 'list'
  } catch (e) { console.error('保存失败:', e); alert('保存失败') }
}
const handleDeleteConfig = async (id: number, event: Event) => { event.stopPropagation(); if (!confirm('确认删除？')) return; try { if (useServer()) { await deleteModel(id); await loadModelsData() } else { models.value = models.value.filter(m => m.id !== id); persistModels() }; delete frontendMeta.value[id]; saveMeta(frontendMeta.value) } catch (e) { console.error('删除失败:', e) } }

const onDragStart = (index: number) => { dragIndex.value = index }
const onDragOver = (e: DragEvent) => { e.preventDefault() }
const onDrop = async (targetIndex: number) => { if (dragIndex.value == null || dragIndex.value === targetIndex) return; const items = [...models.value]; const moved = items.splice(dragIndex.value, 1)[0]; if (!moved) return; items.splice(targetIndex, 0, moved); models.value = items; models.value.forEach((m, i) => { m.sortOrder = i }); if (useServer()) { await reorderModels(models.value.filter(m => m.id != null).map(m => ({ id: m.id!, sortOrder: m.sortOrder ?? 0 }))) } else { persistModels() }; dragIndex.value = null }
  ...
  <div class="rag-container-glass">
    <div v-if="step === 'list'" class="list-layout">
      ...
      <div class="records-grid">
        <div v-for="(item, idx) in models" :key="item.id" class="record-card" :class="{ 'is-enabled': item.isEnabled === 1 }" :style="{ borderLeft: `6px solid ${item.isEnabled === 1 ? getColor(item) : '#cbd5e1'}` }" :draggable="true" @dragstart="onDragStart(idx)" @dragover="onDragOver" @drop="onDrop(idx)">
          <div class="drag-handle" title="拖拽排序">↕</div>
          <div class="card-info" @click="handleToggle(item, $event)">
            <div class="title-row"><span class="toggle-check">{{ item.isEnabled === 1 ? '☑' : '☐' }}</span><h4 :style="{ color: getColor(item) }">{{ getTitle(item) }}</h4><span v-if="item.isEnabled === 1" class="enabled-tag" :style="{ backgroundColor: getColor(item) }">{{ item.sortOrder ?? idx }}</span></div>
            <span class="badge">模型：{{ item.modelName }}</span><span class="badge" style="margin-left:6px">提供商：{{ item.provider }}</span><span v-if="item.displayName" class="badge" style="margin-left:6px;color:var(--theme-color)">别称：{{ item.displayName }}</span>
          </div>
          <div class="card-actions"><button class="edit-btn" @click="handleEditConfig(item, $event)">⚙️</button><button class="delete-btn" @click="handleDeleteConfig(item.id!, $event)">🗑</button></div>
        </div>
        <div v-if="models.length === 0" style="text-align:center;color:#94a3b8;padding:40px;">暂无模型配置</div>
      </div>
    </div>
...
<style scoped>
.rag-container-glass { width: 90%; max-width: 780px; padding: 24px; border-radius: 20px; background: rgba(255,255,255,0.45); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.5); box-shadow: 0 20px 40px rgba(0,0,0,0.02); max-height: 80vh; overflow-y: auto; }
